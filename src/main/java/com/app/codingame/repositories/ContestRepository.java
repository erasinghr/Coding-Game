...
public class ContestRepository implements IContestRepository {
    ...
    public List<Contest> findAllContestLevelWise(Level level) {
        List<Contest> clistfilt = new ArrayList<Contest>();
        List<Contest> clist = new ArrayList<Contest>(contestMap.values());
        clistfilt = clist.stream().filter(i -> i.getLevel() == level).collect(Collectors.toList());
        return clistfilt;
    }
